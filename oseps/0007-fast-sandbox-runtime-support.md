---
title: Fast Sandbox Runtime Integration
authors:
  - "@fengcone"
  - "@Pangjiping"
...
- [Proposal](#proposal)
  ...
  - [Fast Sandbox backend and API reuse model](#fast-sandbox-backend-and-api-reuse-model)
...
**`POST /templates` — `CreateFsbTemplateRequest`** (simplified build intent; `additionalProperties: false`):
...
**`GET /templates`** — `ListFsbTemplatesResponse` (shared pagination envelope, same field semantics as `ListSandboxesResponse`): `items: array[FsbTemplate]`, `page`, `pageSize`, `totalItems`, `totalPages`, `hasNextPage` (all required). Query parameters mirror the sandbox list contract:
