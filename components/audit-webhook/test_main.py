...
class FakeStore:
    ...
    def list_latest(self, search=None, sort="-request_time", limit=50, offset=0):
        if self.fail:
            raise RuntimeError("db down")
        events = [
            event for event in self.events
            if search is None or search.lower() in event["sandbox_id"].lower()
        ]
        return {
            "total": len(events),
            "items": [
...
def test_record_single_event(client):
    ...
    assert fake.events[0]["request_time"].isoformat() == "2026-08-20T09:24:12.252000+08:00"
...
def test_index_page(client):
    ...
    assert "/details?sandbox_id=" in response.text
