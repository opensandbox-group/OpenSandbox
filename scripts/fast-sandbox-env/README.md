...
- **Pool with egress**: `firecracker-egress-pool` (firecracker runtime,
  ...
  `actionHandlers` egress@18080 with the runtime-ready / data-plane-ready
  hooks). The egress image is built from `components/egress` in this
  repository and kind-loaded.
- **P2P by default**: two kind nodes, each running the runtime-agent with
  ...
  Service); the pool spreads one fastlet per node via podAntiAffinity, so
  artifact pulls flow `cache -> peer -> origin` with the origin fetched
  ~once per 4MiB block cluster-wide. `KIND_SINGLE=1` falls back to one
  node (cache-only).
...
manifests/
  cluster/kind-cluster.yaml    two-node kind cluster, KVM/tun/shm/StateRoot mounts
  node/firecracker-installer.yaml  firecracker + jailer + kernel onto node hostPath
...
| `MINIO_PORT` / `MINIO_AK` / `MINIO_SK` | `9000` / dev literals | artifact store credentials |
| `IMAGE_<COMPONENT>` | `fast-sandbox/<component>:dev` | fast-sandbox image tags |
