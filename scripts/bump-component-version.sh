...
# For ingress, also updates gateway image tag in manifests/charts/opensandbox-server/values.yaml.
#
# External image pins:
...
if [ "$COMPONENT" = "nodeagent" ]; then
  ...
  CHART_VALUES="manifests/charts/opensandbox-node-agent/values.yaml"
...
CHART_VALUES="manifests/charts/opensandbox-server/values.yaml"
