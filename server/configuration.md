...
| `execd_image` | string | — | OCI image containing the **execd** binary used to bootstrap command/file access inside the sandbox. Docker/Kubernetes run it in-sandbox; the fleets runtime injects it into SandboxTemplate golden-image builds. |
...
### fleets (fast-sandbox) settings under `[kubernetes]`

The fleets backend shares the `[kubernetes]` block; the kubernetes runtime also serves fleets (`flt-`) sandboxes side by side, so these fields are always available.
...
For `runtime.type = "fleets"` the `[kubernetes]` block may be omitted entirely (defaults apply); its `namespace` also selects where fleets sandboxes are created and defaults to `"default"` when unset.
