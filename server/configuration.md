...
- They are dropped with a warning rather than rejected, so existing callers keep working. Nothing that used to be exported stops being exported either: without this endpoint the sidecar had nowhere to export, so those values only ever labelled its own logs. Operators who want extra attributes should set them on the collector, where they belong.
