...
- Setting it makes the sidecar's telemetry identity **server-controlled**. Two per-request keys shape what it reports — `OPENSANDBOX_EGRESS_METRICS_EXTRA_ATTRS` (attached verbatim to every metric) and `OPENSANDBOX_EGRESS_SANDBOX_ID` (used as the `sandbox_id` attribute) — and both are in `ALLOWED_EGRESS_ENV_VARS`. Once metrics land in the operator's backend, a create request could otherwise write arbitrary labels into it and report under another sandbox's identity. So neither is forwarded, and `sandbox_id` is pinned to the real one, which the server knows and the caller does not choose.
- They are dropped with a warning rather than rejected, so existing callers keep working. Operators who want extra attributes should set them on the collector, where they belong.
- Note this makes the two mutually exclusive: a caller-labelled sidecar or a server-managed collector, not both. In a deployment where the sidecar had no endpoint at all these values only ever labelled its own logs, but the shared client also falls back to a node-local collector (`HOST_IP`, then `/etc/hostinfo`), so if your platform injects either out of band the attributes were reaching metrics as well and will stop once `otlp_endpoint` is set.
