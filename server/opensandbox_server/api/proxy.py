...
def _build_proxy_target_url(
    ...
    *,
    ...
    The raw query string is appended to the URI verbatim for both HTTP and
    WebSocket. Handing it to httpx via ``params=`` would re-parse and
    re-serialize it, which reorders repeated keys, adds ``=`` to bare keys,
    and replaces percent-escapes that are not valid UTF-8 (#1901).
    """
